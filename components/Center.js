  ...
  return (
    <div className="flex-grow h-screen overflow-y-scroll scrollbar-hide select-none relative">
      ...
      {session ? (
        <>
          <header className="absolute top-5 right-8" onClick={handleToggle}>
            <div className="flex items-center bg-[#2e2e2e] space-x-3 opacity-90 hover:opacity-80 cursor-pointer rounded-full p-1">
              <img
          ...
          <div
            className={
              ` rounded-full bg-[#2e2e2e] text-white absolute right-8 top-[4.4rem] flex-col` +
              " " +
              `${isActive ? "hidden" : "flex"}`
            }
          >
            <div
              className="flex items-center justify-between rounded-full cursor-pointer px-6 py-2 "
              onClick={signOut}
            >
      ...
      {viewplaylistdata == null ? (
        <h2 className="text-white">Loading... Select a playlist</h2>
      ) : (
        <>
          <section
            className={`flex items-end space-x-7 bg-gradient-to-b to-black ${colors[0]} h-80 text-white pl-5 pb-5 `}
          >
            <img
